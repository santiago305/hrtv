...
export function DataTableResponsiveCards<T extends Record<string, unknown>>({
    ...
    if (data.length === 0) {
        return <div className="rounded-2xl border border-border bg-background px-4 py-10 text-center text-sm text-muted-foreground md:hidden">{emptyMessage}</div>;
    }
    ...
    return (
        <div className="space-y-3 md:hidden">
            {data.map((row, index) => {
                            ...
                            <div className="mb-4">
                                ...
                                <div className="mt-1 text-base font-semibold text-foreground">
                                    {titleColumn.cardCell?.(row, index) ??
                                        ...
                                        (titleColumn.accessorKey ? String(row[titleColumn.accessorKey] ?? '') : null)}
                                </div>
                            </div>
                        ) : null}
                            ...
                            {detailColumns.map((column) => (
                                <div key={column.id} className="flex items-start justify-between gap-3 border-t border-border/60 pt-3 first:border-t-0 first:pt-0">
                                    ...
                                    <div className="text-right text-sm text-foreground">
                                        {column.cardCell?.(row, index) ?? column.cell?.(row, index) ?? (column.accessorKey ? String(row[column.accessorKey] ?? '') : null)}
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                if (!animated) {
                    return (
                        <div key={resolveRowKey(row, index)} onClick={rowClickable ? () => onRowClick?.(row, index) : undefined}>
                            {content}
                        </div>
                    );
                }
